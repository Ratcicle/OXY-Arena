...
void main() {
    ...
    float edgeMask = smoothstep(0.32, 0.96, pow(fresnel, 1.1));
    float alpha = progress * edgeMask;
    vec3 edgeColor = modulatedColor.rgb * lightMapColor.rgb;
    vec4 color = vec4(edgeColor, alpha);
